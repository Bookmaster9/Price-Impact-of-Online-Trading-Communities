...
for id in valid_ids:
    ...
    for offset in ["-2", "-1", "0", "+1", "+2"]:
        weighted_avg_price[offset] = ((data[offset]["bid"]*data[offset]["bid_size"])+(data[offset]["ask"]*data[offset]["ask_size"]))/(data[offset]["bid_size"]+data[offset]["ask_size"])
...
for offset in ["-2", "-1", "0", "+1", "+2"]:
    ...
    np.save(f"processedavgbidask/avg_change_{offset}.npy", avg_change[offset])
    np.save(f"processedavgbidask/std_change_{offset}.npy", std_change[offset])
np.save("processedavgbidask/valid_id_order.npy", np.array(valid_id_order))
